...
import moves from "../data/moves2.json";
import soheiLogo from "/Image1.png";
import NavMenu from "../components/NavMenu";
import "../App.scss";

function FindTheMove2() {
  const { id } = useParams();
  ...
  const goToNext = useCallback(() => {
    if (moveId < moves.length) navigate(`/find-the-move2/${moveId + 1}`);
  }, [moveId, navigate]);

  const goToPrevious = useCallback(() => {
    if (moveId > 1) navigate(`/find-the-move2/${moveId - 1}`);
  }, [moveId, navigate]);
